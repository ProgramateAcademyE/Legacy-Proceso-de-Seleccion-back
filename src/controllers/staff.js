...
staffRouter.get("/interview/:id", async (req, res, next) => {
	const candidateInterview = await InterviewDay.find({
...
staffRouter.get("/assesment/:id", async (req, res, next) => {
	const candidateAssesment = await InterviewDay.find({
		user_id: req.params.id,
	});
...
staffRouter.put("/update-interview", async (req, res) => {
	try {
		...
		if (interview) {
			const interviewScore = await InterviewDay.updateMany(
				{ user_id: user_id },
				{
					$set: {
						interview: interview,
			...
			// REFACTORIZAR
			resultInterview =
				(motivation.question1.score +
					motivation.question2.score +
					perseverance.question1.score +
					perseverance.question2.score +
					planning.question1.score +
					planning.question2.score +
					tecnique.question1.score +
					tecnique.question2.score) /
				8;
			const score = await InterviewDay.updateMany(
				{ user_id: user_id },
				{
					$set: {
		...
		if (assesment) {
			const assesmentScore = await InterviewDay.updateMany(
				{ user_id: user_id },
				{
					$set: {
						assesment: assesment,
			...
			const { flexibility, comunication, lidership, achievementOrientation } =
				assesment;
			// REFACTORIZAR
			resultAssesment =
				(flexibility.question1.score +
					flexibility.question2.score +
					comunication.question1.score +
					comunication.question2.score +
					lidership.question1.score +
					lidership.question2.score +
					achievementOrientation.question1.score +
					achievementOrientation.question2.score) /
				8;
			const score = await InterviewDay.updateMany(
				{ user_id: user_id },
				{
